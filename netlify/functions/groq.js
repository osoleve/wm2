...
exports.handler = async (event, context) => {
  ...
  try {
    const { messages, model = "moonshotai/kimi-k2-instruct" } = JSON.parse(event.body);
    ...
    return {
      ...
      body: JSON.stringify({ error: 'Failed to get response from GROQ', details: error.message }),
    };
  }
};
